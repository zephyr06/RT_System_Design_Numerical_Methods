...
// There are two types of tests, strict deadline test, or period & 2xExecution
// test
...
TEST(FindTaskDoNotNeedOptimize, A1) {
    ...
    int indexExpect = 1;
    int indexActual = Opt_LL::FindTaskDoNotNeedOptimize(
        taskSetNormal, 0, responseTimeInitial, eliminateTol);
...
TEST(FindTaskDoNotNeedOptimize, a1) {
    ...
    eliminateTol = 0.1;
    string path =
        "/home/zephyr/Programming/Energy_Opt_NLP/TaskData/test_n3_v13.csv";
    TaskSet taskSet1 = ReadTaskSet(path, "utilization");
    InitializeGlobalVector(taskSet1.size());
    VectorDynamic initialExecution =
        GetParameterVD<int>(taskSet1, "executionTime");
    double eliminateTol_t = eliminateTol;
    eliminateTol = 3.1;
    int index = Opt_LL::FindTaskDoNotNeedOptimize(taskSet1, 0, initialExecution,
                                                  eliminateTol);
    AssertEqualScalar(0, index, 0.00001, __LINE__);
    eliminateTol = 198.1;
    index = Opt_LL::FindTaskDoNotNeedOptimize(taskSet1, 0, initialExecution,
                                              eliminateTol);
...
TEST(FindTaskDoNotNeedOptimize, a2) {
    ...
    exactJacobian = 0;
    string path =
        "/home/zephyr/Programming/Energy_Opt_NLP/TaskData/test_n10_v8.csv";
    TaskSet taskSet1 = ReadTaskSet(path, "RM");
    InitializeGlobalVector(taskSet1.size());
    VectorDynamic initialExecution =
        GetParameterVD<int>(taskSet1, "executionTime");
    initialExecution << 10.7515, 84.5303, 1232.2664, 96.768, 34.85, 243.83,
        600.18, 305.87, 25.12, 37.92;
    UpdateTaskSetExecutionTime(taskSet1, initialExecution);
    int index =
        Opt_LL::FindTaskDoNotNeedOptimize(taskSet1, -1, initialExecution, 0.1);
...
TEST(NumericalDerivativeDynamic, A1) {
    ...
    MatrixDynamic H_Actual =
        NumericalDerivativeDynamic(f, executionTimeVector, deltaOptimizer, 4);
...
TEST(UpdateTaskSetExecutionTime, A1) {
    ...
    if (not(80 == taskSet1[2].executionTime)) throw;
}
...
TEST(unitOptimization, a1) {
    ...
    VectorDynamic res1 =
        Opt_LL::UnitOptimization(taskSetNormal, lastTaskDoNotNeedOptimize,
                                 initialEstimate, responseTimeInitial);

    // 204 corresponds to RT of 319, which should be the best we can get because
    // of the clamp function
...
TEST(OptimizeTaskSet, a1) {
    ...
    string path =
        "/home/zephyr/Programming/Energy_Opt_NLP/TaskData/test_n3_v4.csv";
    // string path =
    // "/home/zephyr/Programming/Energy_Opt_NLP/TaskData/test_data_N5_v2.csv";
    // string path =
    // "/home/zephyr/Programming/Energy_Opt_NLP/TaskData/test_n10_v2.csv";
    // string path =
    // "/home/zephyr/Programming/Energy_Opt_NLP/TaskData/test_n20_v1.csv";
    ...
    if (debugMode == 1) cout << "The energy saving ratio is " << res << endl;
...
TEST(OptimizeTaskSetOneIte, a2) {
    ...
    // string path =
    // "/home/zephyr/Programming/Energy_Opt_NLP/TaskData/test_n20_v1.csv";
    if (debugMode == 1) cout << endl << path << endl << endl;
    TaskSet taskSet1 = ReadTaskSet(path, "utilization");
    InitializeGlobalVector(taskSet1.size());
    weightEnergy = 1e3;
    optimizerType = 1;
    ...
    if (not assert_equal<double>(0.28, res, 0.1))
        CoutError("One test case failed in performance!");
    if (debugMode == 1)
        cout << "The energy saving ratio in "
                "OptimizeTaskSet-OptimizeTaskSetOneIte is "
             << res << endl;
}
...
TEST(ClampComputationTime, a1) {
    ...
    // 204 corresponds to RT of 319, which should be the best we can get because
    // of the clamp function if (not(abs(205 - res1(0, 0)) < 1))
    //     CoutWarning("One test case failed in performance!");
...
TEST(ClampComputationTime, v3) {
    ...
    TaskSetNormal taskSetNormal(tasks);
    Opt_LL::ClampComputationTime(taskSetNormal, lastTaskDoNotNeedOptimize,
                                 responseTimeInitial, "fine");
    VectorDynamic expect1 = initialEstimate;
    expect1 << 15, 13, 17, 24, 19;
    EXPECT(assert_equal(expect1, GetParameterVD<double>(taskSetNormal.tasks_,
                                                        "executionTime")));
}
TEST(ClampComputationTime, v2) {
    ...
    MaxComputationTimeRestrict = 2;
    string path =
        "/home/zephyr/Programming/Energy_Opt_NLP/TaskData/test_n5_v22.csv";
    TaskSet tasks = ReadTaskSet(path, "orig");
    TaskSetNormal taskSetNormal(tasks);
    InitializeGlobalVector(tasks.size());
    int N = tasks.size();
...
TEST(UnitOptimizationIPM, a1) {
    ...
    MaxComputationTimeRestrict = 100;
    string path =
        "/home/zephyr/Programming/Energy_Opt_NLP/TaskData/test_n3_v21.csv";
    TaskSet tasks = ReadTaskSet(path, "RM");
    TaskSetNormal taskSetNormal(tasks);
    VectorDynamic initialExecution =
        GetParameterVD<int>(tasks, "executionTime");
    eliminateTol = 1;
    optimizerType = 1;
    EnergyMode = 1;
    enableMaxComputationTimeRestrict = 0;
    ...
    VectorDynamic res =
        Opt_LL::UnitOptimization(taskSetNormal, 1, initial, initialExecution);
...
// {
//     string pathInPeriodicDataset =
//     "/home/zephyr/Programming/Energy_Opt_NLP/TaskData/task_number/periodic-set-0-syntheticJobs.csv";
//     int N = 20;
//     string yechengRepoPath =
//     "/home/zephyr/Programming/others/YechengRepo/Experiment/WCETEnergyOpt/TestCases/NSweep/N"
//     + to_string(N) + "/";

//     int index = 0;
//     string targetFilePathGP = yechengRepoPath + "Case" + to_string(index) +
//     ".txt" + "_RM_GPResult.txt"; string targetFilePathBF = yechengRepoPath +
//     "Case" + to_string(index) + ".txt" + "_RM_BFSResult.txt"; if (debugMode
//     == 1)
...
//     else
//     {
//         CoutError("Unrecognized baselineLLCompare! Current value is " +
//         to_string(baselineLLCompare));
//     }
...
TEST(EnergyFactor, v1) {
    string path =
        "/home/zephyr/Programming/Energy_Opt_NLP/TaskData/test_data_N3.csv";
    ...
    MatrixDynamic h1Expect =
        GenerateVectorDynamic1D(-2 / tasks[0].period * weightEnergy);
    MatrixDynamic h2 = GenerateMatrixDynamic(1, 1);
    MatrixDynamic h2Expect =
        GenerateVectorDynamic1D(-2 / tasks[1].period * weightEnergy);
    AssertEqualScalar(
        3777777.7778,
        f0.evaluateError(GenerateVectorDynamic1D(tasks[0].executionTime), h1)(
            0, 0));
    AssertEqualScalar(
        2448979.5918,
        f1.evaluateError(GenerateVectorDynamic1D(tasks[1].executionTime), h2)(
            0, 0));
...
TEST(RTARelatedFactor, v1) {
    ...
    auto f1 = GenerateRTARelatedFactor<TaskSetNormal, RTA_LL>(
        tasksN, eliminationRecord);
    ...
    for (int i = 0; i < 5; i++) assert_equal(HsExpect[i], Hs[i]);
}
