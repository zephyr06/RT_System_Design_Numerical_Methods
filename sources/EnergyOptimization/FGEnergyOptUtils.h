...
static gtsam::Values GenerateInitialFG(TaskSet tasks,
...
static gtsam::Values GenerateInitialFG(TaskSetNormal tasks,
                                       EliminationRecord &eliminationRecord) {
    return GenerateInitialFG(tasks.tasks_, eliminationRecord);
}

VectorDynamic ExtractResults(const gtsam::Values &result,
                             const TaskSet &tasks) {
    VectorDynamic executionTimes =
        GetParameterVD<double>(tasks, "executionTime");
    for (uint i = 0; i < tasks.size(); i++) {
        if (result.exists(GenerateKey(i, "executionTime"))) {
            executionTimes(i, 0) =
                result.at<VectorDynamic>(GenerateKey(i, "executionTime"))(0, 0);
        }
    }
    return executionTimes;
}
VectorDynamic ExtractResults(const gtsam::Values &result,
                             const TaskSetNormal &tasks) {
