...
OptimizeResult OptimizeSchedulingSA(TaskSetType &tasks) {
    ...
    if (!r.CheckSchedulabilityDirect(responseTimeInitial))
        return {INT_MAX, INT_MAX, initialEstimate, initialEstimate};
    gtsam::Symbol key('a', 0);
    auto model = gtsam::noiseModel::Isotropic::Sigma(
        (N - lastTaskDoNotNeedOptimize - 1), noiseModelSigma);
    typename Energy_Opt<TaskSetType, Schedul_Analysis>::ComputationFactor
        factor(key, tasks, lastTaskDoNotNeedOptimize, responseTimeInitial,
               model);

    moe::SimulatedAnnealing<double> moether(
    ...
    moether.setFitnessFunction([&](auto startTimeVec) -> double {
        VectorDynamic startTimeVector =
            Vector2Eigen<double>(startTimeVec.genotype);
        VectorDynamic err;
    ...
    if (debugMode == 1) {
        std::cout << "Initial estimation for SA is " << initialEstimate
                  << std::endl;
        std::cout << "fitness: " << best_moe.fitness * -1 << "\n"
