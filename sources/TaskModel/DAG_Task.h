...
// this file implements standard DAG Task Models
...
void AddVertexMy(Graph &g, IndexVertexMap &indexesBGL, int taskIndex) {
    ...
    boost::tie(pos, inserted) =
        indexesBGL.insert(std::make_pair(taskIndex, Vertex()));
    if (inserted) {
        u = add_vertex(g);
...
class DAG_Model {
   public:
    ...
    DAG_Model(TaskSet &tasks, Graph &graph, IndexVertexMap &index2Vertex,
              Vertex_name_map_t &vertex2index)
    ...
    /**
     * @brief add a dummy node to graph for the convenience of analysis by
     * Nasri19; this should only be applied during RTA analysis
     *
     */
    void AddDummyNode() {
...
DAG_Model ReadDAG_Task(std::string path, std::string priorityType = "orig") {
    ...
    if (file.is_open()) {
        ...
        if (debugMode == 1)
            std::cout << "Finish reading the data file" << path
                      << " succesfully!\n";
        return dagModel;
    } else {
...
std::vector<int> GetDependentTasks(DAG_Model &dagTasks, int index) {
    ...
    for (boost::tie(ei, edge_end_i) = in_edges(v, dagTasks.graph_);
         ei != edge_end_i; ++ei) {
