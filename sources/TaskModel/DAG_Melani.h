...
// the DAG model is from the paper "Response-Time Analysis of Conditional DAG
// Tasks in Multiprocessor Systems, Meltani15ECRTS"
...
struct TaskSetDAG : public TaskSetNormal {
   public:
    ...
    TaskSetDAG(const TaskSet &tasks, std::vector<double> &volumeVec,
               std::vector<double> &longestVec, std::vector<double> &weight)
        : TaskSetNormal(tasks),
          volumeVec_(volumeVec),
    ...
    if (file.is_open()) {
        ...
        while (getline(file, line)) {
            ...
            if (dataInLine.size() < 9)
                CoutError(
                    "The path in ReadDAG_Tasks doesn't follow Melani format!");
