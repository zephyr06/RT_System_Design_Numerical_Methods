// This function provides several helper functions for unit tests
#pragma once
...
void AssertUnEqual(T expect, T actual, int lineNumber = 0) {
    if (lineNumber != 0)
        std::cout << Color::red << "Line Number: " << std::to_string(lineNumber)
                  << Color::def << std::endl;
    std::cout << "Assertion failed!" << std::endl;
    std::cout << Color::red << "EXpect is " << expect
              << ", while the actual is " << actual << Color::def << std::endl;
    throw;
}
...
template <typename T>
void AssertEqualVectorNoRepeat(const std::vector<T> &expected,
    ...
    for (size_t i = 0; i < expected.size(); i++) s.insert(expected.at(i));
    for (size_t i = 0; i < expected.size(); i++) {
        if (s.find(actual.at(i)) == s.end()) {
            CoutError("Actual element " + to_string(actual.at(i)) +
                      " is not found in expected vector");
        } else {
            s.erase(actual.at(i));
        }
    }
...
template <typename T>
void AssertEqualVectorExact(const std::vector<T> &expected,
                            const std::vector<T> &actual,
                            double tolerance = 1e-6, int lineNumber = 0) {
    if (expected.size() != actual.size()) {
        std::cout << Color::red << "Length error! " << Color::def;
...
void AssertEqualMap(std::unordered_map<T1, T2> &mExpect,
                    std::unordered_map<T1, T2> &mActual) {
    using namespace std;
    ...
    for (auto itr = mExpect.begin(); itr != mExpect.end(); itr++) {
        ...
        if (itrActual == mActual.end() ||
            (itrActual->second).notEqual(itr->second)) {
            try {
                std::cout << "Expect is " << itr->first << ", " << itr->second
                          << endl;
                std::cout << "Actual is " << itrActual->first << ", "
                          << itrActual->second << endl;
            } catch (const std::exception &e) {
                std::cout << "Cannot print the key to show mismatch element\n";
            }
            CoutError(
                "Element in mExpect is not found in or not equal to mActual!");
        }
    }
}
