 ...
 * @return double
 */
double HingeLoss(double x) { return max(0, -1 * x); }
...
class InequalityFactor1D : public gtsam::NoiseModelFactor1<VectorDynamic> {
   public:
     ...
     */
    InequalityFactor1D(gtsam::Key key, gtsam::SharedNoiseModel model)
        : gtsam::NoiseModelFactor1<VectorDynamic>(model, key) {
        index = -1;
        dimension = 1;
    }

    InequalityFactor1D(gtsam::Key key, int index, gtsam::SharedNoiseModel model)
        : gtsam::NoiseModelFactor1<VectorDynamic>(model, key), index(index) {
        dimension = 1;
    }

    InequalityFactor1D(gtsam::Key key, NormalErrorFunction1D f,
                       gtsam::SharedNoiseModel model)
        : gtsam::NoiseModelFactor1<VectorDynamic>(model, key), f(f) {
        index = -1;
        dimension = 1;
    }
    ...
    gtsam::Vector evaluateError(
        ...
        if (H) {
            if (exactJacobian) {
                *H =
                    NumericalDerivativeDynamic(f, x, deltaOptimizer, dimension);
            } else {
...
class SmallerThanFactor1D : public InequalityFactor1D {
   public:
    double b;
    SmallerThanFactor1D(gtsam::Key key, double b, gtsam::SharedNoiseModel model)
        : InequalityFactor1D(key, model) {
    ...
    SmallerThanFactor1D(gtsam::Key key, double b, int indexInEliminationRecord,
                        gtsam::SharedNoiseModel model)
        : InequalityFactor1D(key, indexInEliminationRecord, model) {
        f = [b](const VectorDynamic &x) {
            VectorDynamic res = x;
            res << Barrier(b - x(0, 0));
            return res;
...
class LargerThanFactor1D : public InequalityFactor1D {
   public:
    double b;
    LargerThanFactor1D(gtsam::Key key, double b, gtsam::SharedNoiseModel model)
        : InequalityFactor1D(key, model) {
        f = [b](const VectorDynamic &x) {
            VectorDynamic res = x;
            res << Barrier(x(0, 0) - b);
 ...
 */
    ...
    InequalityFactor2D(gtsam::Key key1, gtsam::Key key2,
                       NormalErrorFunction2D f, gtsam::SharedNoiseModel model)
